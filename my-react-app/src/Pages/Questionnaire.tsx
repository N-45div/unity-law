...
const DivWithDropdown: React.FC<DivWithDropdownProps> = ({
    ...
    useEffect(() => {
      ...
      } else if (highlightedTexts[index] === sickPayClause) {
          setSelectedType("Radio");
          onTypeChange(index, "Radio");
          if (questionText === "No text selected" || questionText === initialQuestionText) {
              setQuestionText("Is the company sick pay applicable?");
              onQuestionTextChange(index, "Is the company sick pay applicable?");
          }
      } else {
